...
export default function Home() {
  ...
  // Movimiento editorial: el scroll sólo revela bloques y orienta la navegación;
  // nunca interviene en agenda, modales, reserva ni contenidos clínicos.
  useEffect(() => {
    let animationFrame = 0;
    const updateHeader = () => {
      const maximum = Math.max(document.documentElement.scrollHeight - window.innerHeight, 1);
      setHeaderActive(window.scrollY > 24);
  ...
  useEffect(() => {
    const blocks = Array.from(document.querySelectorAll<HTMLElement>("[data-nr-reveal]"));
    ...
    blocks.forEach((block) => block.classList.add("nr-reveal-ready"));
  ...
  return (
    <main className="nr-site">
      <header className={`nr-header${headerActive ? " is-scrolled" : ""}`}>
        <span className="nr-header-progress" aria-hidden="true" style={{ transform: `scaleX(${scrollProgress})` }} />
        <Wordmark compact />
      ...
      <section id="inicio" className="nr-hero">
        ...
        <div className="nr-hero-grid">
          <div className="nr-hero-logo-stage nr-hero-stage nr-hero-stage--logo"><img className="nr-hero-logo-image" src={LOGO_MASK_URL} alt="Natalia Rodríguez Studio" /></div>
          <div className="nr-hero-copy">
            <div className="nr-kicker nr-hero-stage nr-hero-stage--kicker"><Sparkles size={14} /> Retiro de acrocordones <span>·</span> Valparaíso, Chile</div>
            <h1 className="nr-hero-stage nr-hero-stage--title">{publishedCopy?.heroTitle || <>Tu piel,<br /><em>tu momento.</em></>}</h1>
            <p className="nr-hero-description nr-hero-stage nr-hero-stage--description">{publishedCopy?.heroSubtitle || "Una atención cercana, técnica y delicada para que vuelvas a sentirte cómoda en tu propia piel."}</p>
            <div className="nr-hero-cta-row nr-hero-stage nr-hero-stage--actions"><BookingCTA tone="dark" onClick={() => setBookingOpen(true)}>Reservar con Natalia</BookingCTA><a className="nr-text-link" href="#servicios">Conocer servicios <ArrowDownRight size={17} /></a></div>
          </div>
          <aside className="nr-hero-note nr-hero-stage nr-hero-stage--agenda" aria-label="Agenda curada">
          ...
          <div className="nr-hero-scroll nr-hero-stage nr-hero-stage--scroll">Desliza para descubrir <span>↓</span></div>
        </div>
      </section>
      ...
      <section id="servicios" className="nr-section nr-offer">
        <div className="nr-section-heading" data-nr-reveal><div><p className="nr-eyebrow">01 · SERVICIOS</p><h2>Un espacio para<br /><em>sentirte bien.</em></h2></div><p>Elige la atención que necesitas. Todo comienza con una conversación clara y una agenda pensada para ti.</p></div>
        <div className="nr-bento nr-offer-bento">
          <article className="nr-bento-service" data-nr-reveal data-nr-delay="0">
          ...
          <figure className="nr-bento-image nr-bento-image--texture" data-nr-reveal data-nr-delay="70"><img src={TEXTURE_URL} alt="Textura editorial rosada de Natalia Rodríguez Studio" /></figure>
          <article className="nr-bento-process" data-nr-reveal data-nr-delay="140"><p className="nr-eyebrow">Tu reserva</p><ol><li><span>01</span><strong>Elige tu atención</strong></li><li><span>02</span><strong>Escoge tu horario</strong></li><li><span>03</span><strong>Confirma de forma segura</strong></li></ol><BookingCTA tone="pink" onClick={() => setBookingOpen(true)}>Reservar ahora</BookingCTA></article>
          <figure className="nr-bento-image nr-bento-image--orbit" data-nr-reveal data-nr-delay="210"><img src={ORBIT_URL} alt="Composición abstracta rosada" /><figcaption>Delicadeza<br />en cada detalle.</figcaption></figure>
        </div>
      </section>

      <section id="sobre-mi" className="nr-section nr-about">
        <div className="nr-about-image" data-nr-reveal><img src={PORTRAIT_URL} alt="Natalia Rodríguez, enfermera estética" /><div className="nr-about-stamp"><span>NR</span><small>STUDIO</small></div></div>
        <div className="nr-about-copy" data-nr-reveal data-nr-delay="90"><p className="nr-eyebrow">02 · CONOCE A NATALIA</p><h2>Tu piel merece una mirada<br /><em>profesional y cercana.</em></h2>{publishedCopy?.about ? <p className="nr-body-copy">{publishedCopy.about}</p> : aboutContent.paragraphs.map((paragraph) => <p className="nr-body-copy" key={paragraph}>{paragraph}</p>)}<div className="nr-check-list">{aboutContent.points.map((point) => <p key={point}><Check size={17} />{point}</p>)}</div><a className="nr-text-link nr-text-link--dark" href={bookingUrl} target="_blank" rel="noreferrer">Hablar con Natalia <ArrowUpRight size={17} /></a></div>
      </section>

      <section id="resultados" className="nr-section nr-results">
        <div className="nr-results-title" data-nr-reveal><p className="nr-eyebrow">03 · PRIVACIDAD Y RESULTADOS</p><h2>Todo cuidado comienza<br /><em>con confianza.</em></h2><p>Las fotografías de resultados se comparten solo con autorización. Mientras incorporamos nuevos casos, puedes conocer el trabajo de Natalia en Instagram.</p><a className="nr-instagram" href={INSTAGRAM_URL} target="_blank" rel="noreferrer"><Instagram size={21} /><span>Ver el trabajo de Natalia</span><ArrowUpRight size={18} /></a></div>
        <div className="nr-results-grid">{gallerySlots.map((slot, index) => <article key={slot.caseLabel} className={`nr-result-card nr-result-card--${index + 1}`} data-nr-reveal data-nr-delay={String(index * 70)}><span>{slot.caseLabel}</span><ShieldCheck size={20} /><strong>{slot.state}</strong><p>{slot.note}</p></article>)}</div>
      </section>

      <section id={COURSE_SECTION_ID} className="nr-section nr-course">
        <div className="nr-course-image" data-nr-reveal><img src={ORBIT_URL} alt="Textura temporal de formación profesional" /></div>
        <div className="nr-course-copy" data-nr-reveal data-nr-delay="90"><p className="nr-eyebrow">04 · FORMACIÓN PROFESIONAL</p><GraduationCap size={31} /><h2>Aprende desde una mirada<br /><em>clara y aplicada.</em></h2><p>{courseContent.description}</p><div className="nr-course-details"><span><b>Dirigido a</b>{courseContent.audience}</span><span><b>Modalidad</b>{courseContent.modality}</span></div><BookingCTA tone="dark" onClick={() => setBookingOpen(true)}>Conocer formación</BookingCTA></div>
      </section>

      <section id="preguntas" className="nr-section nr-faq">
        <div className="nr-faq-intro" data-nr-reveal><p className="nr-eyebrow">05 · PREGUNTAS FRECUENTES</p><h2>Antes de reservar,<br /><em>conversemos.</em></h2><p>Estamos aquí para que agendes con toda la información que necesitas.</p><a className="nr-text-link nr-text-link--dark" href={questionUrl} target="_blank" rel="noreferrer">Hacer una consulta <ArrowUpRight size={17} /></a></div>
        <div className="nr-accordion" data-nr-reveal data-nr-delay="90">{faqContent.items.map((item, index) => <details key={item.question}><summary><span>0{index + 1}</span>{item.question}<ChevronDown size={19} /></summary><p>{item.answer}</p></details>)}</div>
      </section>

      <section className="nr-final-cta"><div className="nr-final-content" data-nr-reveal><p className="nr-eyebrow">TU PRÓXIMA ATENCIÓN</p><h2>Haz espacio para<br /><em>sentirte bien.</em></h2><p>Revisa la agenda disponible y reserva tu atención sin salir del sitio.</p><BookingCTA tone="dark" onClick={() => setBookingOpen(true)}>Reservar mi hora</BookingCTA></div><div className="nr-final-visual" data-nr-reveal data-nr-delay="90"><img src={TEXTURE_URL} alt="Textura rosada abstracta" /><div className="nr-final-bubble">Tu<br />momento<br />empieza<br />aquí.</div></div></section>
